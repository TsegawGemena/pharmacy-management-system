    ...
    <div className="bg-white rounded-xl border border-slate-200/90 p-5 shadow-2xs flex flex-col justify-between h-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Revenue trends in ETB</p>
        </div>

        {/* Timeframe Pill Switcher */}
        <div className="inline-flex rounded-lg border border-slate-200 bg-white p-1 text-xs font-medium shadow-2xs">
          {(["Today", "Week", "Month"] as TimeRange[]).map((tab) => (
            <button
              ...
              className={`px-3 py-1 rounded-md transition-all ${
                range === tab
                  ? "bg-sky-50 text-sky-700 font-semibold border border-sky-200/60 shadow-2xs"
      ...
      <div className="relative w-full pt-2">
        ...
        {hoveredPoint && (
          <div
            className="absolute z-20 pointer-events-none -translate-x-1/2 -translate-y-full mb-2 bg-slate-900 text-white text-xs py-1 px-2.5 rounded shadow-lg flex flex-col items-center"
            style={{
        ...
        {/* X-axis labels */}
        <div className="flex justify-between px-2 text-[11px] text-slate-400 font-medium pt-1">
